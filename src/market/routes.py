    ...
    if request.method == "POST":
        ...
        if p_item_object:
            if current_user.can_purchase(p_item_object):
                ...
                flash(f"Congratulation ! you purchased the item {p_item_object.name} for the {p_item_object.price} $",category="success")
            else:
                flash(f"Unfortunately , You dont have enough money to buy this item {p_item_object.name}",category="danger")
    
            return redirect(url_for('market_page'))
    if request.method == "GET":
        items = Item.query.filter_by(owner=None)
        return render_template('market.html',items=items,page='market',purchase_form = purchase_form)
